...
% Last Modified by GUIDE v2.5 11-Jun-2020 02:31:05
...
fileID = fopen('AandB.h','w');
fprintf(fileID,'#ifndef _AandB_H_\n');
fprintf(fileID,'#define _AandB_H_\n');
fprintf(fileID, 'extern const float A[%f], B[%f];\n', length(Ap),length(Bp));
fprintf(fileID, '#endif');
fclose(fileID);

fileID3 = fopen('AandB.c','w');
fprintf(fileID3, '#include \"AandB.h\"\n');
fprintf(fileID3, 'const float A[]=');
fprintf(fileID3, '{%f',Ap(1));
fclose(fileID3);
for ll = 2:length(Ap)
fileID3 = fopen('AandB.c','a');
fprintf(fileID3,',');
fprintf(fileID3,'%f',Ap(ll));
end
fprintf(fileID3, '};\n');
fprintf(fileID3, 'const float B[]=');
fprintf(fileID3, '{%f',Bp(1));
fclose(fileID3);
for ll = 2:length(Bp)
fileID3 = fopen('AandB.c','a');
fprintf(fileID3,',%f ',Bp(ll));
end
fprintf(fileID3, '};\n');
fclose(fileID3);
...
fprintf(fileID, '#define _RST_H_\n');
fprintf(fileID, 'extern const float R[');
fprintf(fileID, num2str(length(R)));
fprintf(fileID, '], S[%d], T[%d];\n',num2str(length(S)),num2str(length(T)) );
fprintf(fileID, '#endif');
fclose(fileID);

fileID2 = fopen('RST.c','w');
fprintf(fileID2, '#include \"RST.h\"\n');
fprintf(fileID2, 'const float R[]=');
fprintf(fileID2, '{');
fprintf(fileID2,'%f',R(1));
fclose(fileID2);
for ll = 2:length(R)
fileID2 = fopen('RST.c','a');
fprintf(fileID2,',');
fprintf(fileID2,'%f',R(ll));
end
fprintf(fileID2, '};\n');
fclose(fileID2);

fileID2 = fopen('RST.c','a');
fprintf(fileID2, 'const float S[]=');
fprintf(fileID2, '{');
fprintf(fileID2,'%f',S(1));
for lll = 2:length(S)
fileID2 = fopen('RST.c','a');
fprintf(fileID2,',');
fprintf(fileID2,'%f',S(lll));
end
fprintf(fileID2, '};\n');
fclose(fileID2);  
...
fileID2 = fopen('RST.c','a');
fprintf(fileID2, 'const float T[]=');
fprintf(fileID2, '{');
fprintf(fileID2,'%f',T(1));
for lll = 2:length(T)
fileID2 = fopen('RST.c','a');
fprintf(fileID2,',');
fprintf(fileID2,'%f',T(lll));
end
fprintf(fileID2, '};\n');
fclose(fileID2); 
...
x=x-195;
add_line(name, 'R(z)/1)', 'Sum1/2');
blocks = find_system(subsystem, 'SearchDepth', 1);
  bh = [];
...
cs = getActiveConfigSet(subsystem);
stf = 'ert.tlc';
tmf = 'ert_default_tmf';
mc  = 'make_rtw';
thd = 'NXP->Cortex-M4';
